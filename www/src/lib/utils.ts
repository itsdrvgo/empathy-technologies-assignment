...
export function handleClientError(error: unknown, tOptions?: ExternalToast) {
    if (error instanceof AxiosError)
        return toast.error(sanitizeError(error), tOptions);
    else if (error instanceof ZodError)
        return toast.error(sanitizeError(error), tOptions);
    else if (error instanceof Error)
        return toast.error(error.message, tOptions);
    else toast.error(DEFAULT_MESSAGES.ERRORS.GENERIC, tOptions);
}
