...
class URLBuilder {
     ...
     *        If undefined is provided, the parameter will be omitted from the URL.
     * @returns The current URLBuilder instance for method chaining.
     * @example
     * builder.addQueryParam('sort', 'date')
     *        .addQueryParam('include', ['comments', 'likes'])
     *        .addQueryParam('filter', { status: 'published' })
     *        .addQueryParam('page', undefined); // This parameter will be omitted
     */
    addQueryParam(key: string, value: QueryParamValue): URLBuilder {
        if (value !== undefined) {
            this.queryParams.set(key, value);
        } else {
            // If the value is undefined, remove the key if it exists
    ...
    private serializeQueryParam(value: QueryParamValue): string {
        ...
        // Check if the value is a URL-like string
        const stringValue = String(value);
        if (stringValue.includes("://")) {
            // For URL-like strings, use encodeURI instead of encodeURIComponent
